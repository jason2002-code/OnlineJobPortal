...
// Fetch notifications for the logged-in jobseeker
$notifications = [];
$unreadCount = 0;
$notifQuery = "SELECT notificationID, message, isRead, dateSent
               FROM notifications
               WHERE user_Id = ? AND receiverRole = 'jobseeker' AND (isHidden IS NULL OR isHidden = 0)
...
if (isset($_GET['notificationID'])) {
    ...
    $updateReadSql = "UPDATE notifications SET isRead = 1 WHERE notificationID = ? AND user_Id = ? AND receiverRole = 'jobseeker'";
    if ($stmt = $conn->prepare($updateReadSql)) {
        $stmt->bind_param("ii", $notificationID, $jobseeker_id);
        $stmt->execute();
        $stmt->close();
    }
    // Redirect to avoid resubmission
    header("Location: jobseeker_notifications.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hide_notification']) && isset($_POST['notificationID'])) {
    ...
    $hideSql = "UPDATE notifications SET isHidden = 1 WHERE notificationID = ? AND user_Id = ? AND receiverRole = 'jobseeker'";
    if ($stmt = $conn->prepare($hideSql)) {
        $stmt->bind_param("ii", $hideNotificationID, $jobseeker_id);
        $stmt->execute();
        $stmt->close();
...
<head>
    ...
    <title>Jobseeker Notifications</title>
                        ...
                        <tr class="<?php echo $notif['isRead'] == 0 ? 'unread' : ''; ?>">
                            ...
                            <td>
                                <?php if ($notif['isRead'] == 0): ?>
                                    <a href="jobseeker_notifications.php?notificationID=<?php echo $notif['notificationID']; ?>" class="btn btn-sm btn-primary">Mark as Read</a>
                                <?php endif; ?>
                                <form method="post" action="jobseeker_notifications.php" style="display:inline;" onsubmit="return confirm('Hide this notification?');">
